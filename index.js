...
function printIndexes(array) {
  for (let index in array) { // Проходимося по всіх індексах масиву
    console.log(index); 
...
function multiplyArray(array) {
  ...
  for (let value of array) { // Проходимося по всіх значеннях масиву
...
function printUpperCase(array) {
  ...
  for (let i = 0; i < array.length; i++) { // Проходимося по всіх елементах масиву
...
function findMinMax([firstElement, ...restElements]) {
  let min = firstElement; // Ініціалізуємо змінні для мінімального та максимального значень, задаємо їм перше значення масиву
  let max = firstElement; // Перебираємо масив починаючи з другого елементу

  for (let i = 0; i < restElements.length; i++) {
    const currentElement = restElements[i];

  if (currentElement < min) { // Перевіряємо, чи поточний елемент менший за мінімальне значення
    min = currentElement; // Якщо так, оновлюємо мінімальне значення
  }
  ...
  console.log('Min:', min); 
  console.log('Max:', max); // Виводимо мінімальне та максимальне значення
}
...
function printStrings(array) {
  ...
  const repeatedString = string.repeat(2); 
  
  console.log(repeatedString); // Перетворюємо масив в рядок і видаляемо пробіли та виводимо його на консоль
}
...
function concatenateStrings(array, subString) {
  const concatenatedString = array.toString(); // Використовуємо метод toString() для перетворення масиву в рядок
  const containsSubstring = concatenatedString.includes(subString); //Перевіряємо чи містить рядок підрядок
...
function addTenToEach(array) {
  const resultArray = []; // Ініціалізуємо масив через конструктор з такою же кількістю елементів як в вхідному масиві для зберігання результату
  for (let index in array) { // Перебираємо масив за допомогою циклу for-in
    resultArray.push(array[index] + 10);  // Додаємо до поточного елемента 10 та додаємо результат в новий масив
...
function swapMinMax(array) {
  let max = array[0]; // Ініціалізуємо дві змінні (max і min) з першим елементом масиву. Ці змінні будуть використовуватися для зберігання максимального та мінімального значень в масиві.
  let min = array[0];
  let maxIndex = 0;
...
function getEvenNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) { // Проходимо через кожен елемент вхідного масиву
    const num = arr[i]; 
    if (num % 2 === 0) { // Перевіряємо чи є число парним
      result.push(num); // Якщо так, додаємо його до масиву результату
    }
  }
...
function multiplyByIndex(arr) {
  const result = [];  // Створюємо пустий масив для зберігання результату
  for (let i = 0; i < arr.length; i++) { // Проходимо через кожен елемент вхідного масиву
    const multiplyNum = arr[i] * i; // Помножуємо число на його індекс і додаємо до масиву результату
    result.push(multiplyNum); 
  }
  return result; // Повертаємо масив результату
}
...
function replaceNumbers(arr) {
  const result = [];  // Створюємо пустий масив для зберігання результату
  for (let i = 0; i < arr.length; i++) { // Проходимо через кожен елемент вхідного масиву
    if (arr[i] > 10) { // Перевіряємо, чи число більше за 10
      result.push("Greater than 10");  // Якщо число більше за 10, додаємо рядок "Greater than 10" до масиву результату
    } else { 
      result.push("Less than or equal to 10"); // Якщо число менше або дорівнює 10, додаємо рядок "Less than or equal to 10" до масиву результату
    }
  }
